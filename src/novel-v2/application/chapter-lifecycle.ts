...
export async function runChapterLifecycle(params: {
  ...
  /** Review the current candidate before a directed revision when no comparable baseline exists. */
  ...
  while (true) {
    ...
    await params.progress({ stage: "revision-decision", iteration, decision: decision.reason, revisionEvidence: decision.evidence, currentScore: decision.currentScore });
