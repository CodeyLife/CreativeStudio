...
export interface PromptContextManifest {
  ...
  /** maxInputTokens 扣除输出预留、system 和 schema 后，真正可用于 sections 的容量。 */
  usableInputTokens: number;
  /** critical/required sections 连同 system/schema 所需的最低输入预算。 */
  requiredInputTokens: number;
  /** 因预算被排除的条目数，包含上游已在记忆选择阶段标记为 budget 的 receipt。 */
  budgetExcludedSectionCount: number;
...
export type SkillSourceMode = "workspace" | "database";

export type SkillExecutionPoint =
  | "foundation.book-plan"
  | "arc.plan"
  | "arc.review"
  | "arc.revision"
  | "chapter.blueprint"
...
export type SkillPriority = "required" | "normal" | "optional";
...
export interface SkillResolutionManifest {
  ...
  resolvedAt: number;
  /** 解析器选中的完整 Skill 集合；后续模型调用不会以此字段锁定版本。 */
  skills: Array<{
    skillId: string;
    version: string;
    contentFingerprint: string;
  ...
  /** 当前 Prompt 编译或模型调用实际注入的 Skill；旧历史 manifest 可以没有该字段。 */
...
export interface SkillBundle {
  ...
  projectId: string;
  preflightId: string;
  skills: Array<Pick<SkillDescriptor, "skillId" | "version" | "qualityGates" | "promptSections"> & Partial<Pick<SkillDescriptor, "capabilities" | "applicableTasks" | "requiredMemoryKinds" | "conflicts" | "applicableGenres" | "enabled" | "executionPoints" | "roles" | "dependsOn" | "priority" | "contentFingerprint" | "sourceRef">>>;
