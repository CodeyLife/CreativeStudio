...
export function buildFoundationPrompt(input: {
  ...
  const lines = [
    ...
    "叙事语言降噪：主角的独特认知（职业思维、专业训练、天赋等）是能力的设定来源，不是叙事语言。专业术语只允许出现在设定标注（如世界观机制、知识边界、揭示物）与设计意图中；不得写入声部锚点、叙事基调、章节设计等会直接传导到正文的位置。正文应以普通读者可读的叙事呈现——声部用'理性、观察入微、善用类比'等可感特质描述，不用'习惯用职业黑话'这类要求，避免模型在正文中把专业思维解说当成叙事主导。",
    "",
  ...
  lines.push(
    ...
    "表层呈现面向大众读者：所有读者可见的表面（卷名、章节名、概念表述、势力与术语命名）必须落在大众熟悉的知识范围内。专业或技术概念只允许出现在机制层设计与主角认知推理中；若需出现于表面，必须转译为题材通用表达，同一概念全篇保持同一译名。机制‘可推导’不等于表面‘可技术化’：硬核感来自推理可信度，不来自词汇。",
