...
function buildWritingContract(targetWordCount?: number): string {
  return [
    ...
    "视角降噪：主角的独特认知（职业思维、专业训练、天赋等）是设定——解释他为何能看出常人看不到的规律，不是叙事语言。认知优势通过观察、判断、行动与结果自然体现（如'他敏锐地发现局势中常人难以察觉的规律'），不以专业思维解说或职业黑话外显代替动作与场景。专业隐喻仅作适度点缀，每次出现须承担新的信息、选择或世界观功能；正文是普通读者可读的叙事，不是技术解说。",
