...
function reviewInstruction(input: ReviewPromptInput, memory: MemoryBundle): string {
  const lines = [
    ...
    "叙事语言降噪检查（prose-reviewer 主责，其他角色可补充）：主角的独特认知（职业思维、专业训练、天赋等）是设定来源——解释他为何能看出常人看不到的规律——不是叙事语言。若正文把机制层概念以职业黑话原词作为叙事主导，连续用专业术语解说身体感觉、动作或意志（叙述者以专业系统比喻重新命名现象、把身体或情绪状态描述为专业故障），属于叙事语言污染，判为 major 并要求转译为题材通用表达或普通读者可读的类比；只有当单个术语承担不可替代的世界观揭示或角色独有认知时才保留，且不能是当前动作的唯一载体。开篇一次性交代主角出身背景是允许的，但不得以此授权全篇技术解说。角色独有认知豁免有集中度上限：若同一收束段、心理总结或局部节拍连续堆叠多个专业隐喻重新命名同一体验（如以职业术语反复宣告任务完成、障碍清除、谜团得解来确认同一认知收获），即使每个词单看都算角色认知，整体已从点缀变成叙事主导，仍判 major 并要求集中转译为题材通用表达，不能以'表达角色认知'为由整段豁免。",
