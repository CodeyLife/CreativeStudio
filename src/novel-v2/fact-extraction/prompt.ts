...
export function buildFactExtractionPrompt(input: FactExtractionPromptInput): string {
  ...
  sections.push(
    `## 顶层输出合同（不可省略）`,
    `- 顶层必须始终输出 facts、narrativeElements、payoffMoments 三个字段；没有内容时分别输出空数组或包含三个空数组的对象。`,
    `- chapterMemory 和 characterDeltas 不属于本次输出；它们由后续独立 handler 提取，不能嵌套进本对象。`,
    ...
    `- setupDescription 是该爽点的前置铺垫（如"前5章被嘲讽低估"），若无明显铺垫写空字符串。`,
    `- evidence 必须引用正文逐字证据（不少于 8 字），不得概括。`,
