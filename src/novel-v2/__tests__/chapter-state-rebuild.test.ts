...
const factExtractionSkill: SkillDescriptor = {
  skillId: "fact-extraction-test",
  version: "1.0.0",
  capabilities: ["memory"],
  applicableTasks: ["memory-maintenance"],
  requiredMemoryKinds: [],
  conflicts: [],
  qualityGates: [],
  promptSections: { "chapter.fact-extraction": "只提取正文实际呈现且会影响后续连续性的事实。" },
  enabled: true,
  executionPoints: ["chapter.fact-extraction"],
  roles: ["fact-extractor"],
  dependsOn: [],
  priority: "required",
};
