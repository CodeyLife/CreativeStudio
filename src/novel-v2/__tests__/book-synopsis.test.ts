...
describe("book synopsis planning contract", () => {
  ...
  it("materializes external-mcp routing as a claimable model task", async () => {
    ...
    const activities = createNovelWorkflowActivities({
      ...
      skillProvider: { list: vi.fn(async () => [{ skillId: "foundation-test", version: "1.0.0", capabilities: ["planning"], applicableTasks: ["foundation"], requiredMemoryKinds: [], conflicts: [], qualityGates: [], promptSections: { "foundation.book-plan": "以结构化事实完成基础规划。" }, enabled: true, executionPoints: ["foundation.book-plan"], roles: ["planner"], priority: "required" }]) } as unknown as SkillProvider,
