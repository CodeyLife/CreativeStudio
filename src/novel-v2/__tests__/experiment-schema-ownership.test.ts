...
describe("experiment schema ownership", () => {
  it("classifies every migrated V2 table as runtime data or explicit control data", () => {
    ...
    const deletedBySubtractionMigration = new Set(["payoff_curve"]);
    const expectedRuntime = [...new Set(migratedTables.filter((table) => !controlTables.has(table) && !deletedBySubtractionMigration.has(table)))].sort();
