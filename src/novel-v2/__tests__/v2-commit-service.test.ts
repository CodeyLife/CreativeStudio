...
const reviews = [
  review("structure-reviewer", "internal"),
  review("character-reviewer", "independent"),
  review("prose-reviewer", "independent"),
];
...
describe("V2 CommitService", () => {
  it("rejects commits without all three current reviewer roles", async () => {
    const { service } = createService();
    await expect(service.commit({ projectId: "p1", documentId: "doc-1", artifact, reviews: [reviews[0], reviews[2]], structuralReport, baseRevision: 0, idempotencyKey: "k1", text: "正文" })).rejects.toThrow(/完整三类 reviewer/);
    await expect(service.commit({ projectId: "p1", documentId: "doc-1", artifact, reviews: reviews.map((item, index) => index === 2 ? { ...item, artifactFingerprint: "stale" } : item), structuralReport, baseRevision: 0, idempotencyKey: "k1", text: "正文" })).rejects.toThrow(/prose-reviewer/);
  });
  ...
  it("rejects a structurally blocked manuscript even when all reviewers pass", async () => {
    ...
    const text = `${repeated}\n\n${repeated}`;
    await expect(service.commit({ projectId: "p1", documentId: "doc-1", artifact, reviews, structuralReport: inspectManuscript({ text }), baseRevision: 0, idempotencyKey: "k-structural", text })).rejects.toThrow(/完整三类 reviewer|结构/);
  });

  it("persists through the repository only after all three roles pass", async () => {
