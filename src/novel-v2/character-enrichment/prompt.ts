...
export function buildCharacterEnrichmentPrompt(input: CharacterEnrichmentPromptInput): string {
  const sections = [
    ...
    "",
    `## 字段说明`,
    `- characterId：角色的规范 ID。已有角色必须从下方“规范人物目录”原样复制；规范 ID 只用于关联，不是展示名。目录没有对应项时，使用正文中可指认的原文名称，不要把自然语言名称自行翻译成英文标识，也不要用“主角”“反派”等代词。`,
    ...
    `- relationDeltas：本章中该角色的关系变化。targetCharacterId 必须使用规范人物目录中的 ID；目录没有对应项时使用正文中的原文名称。predicate 是关系类型（如"信任""敌对""师徒"），delta 是变化描述（如"从怀疑转为信任"）。若无变化返回空数组。`,
    "",
    `## 提取规则`,
    ...
    "",
    `## 规范人物目录与已有角色档案（仅供理解上下文，只提取增量）`,
    `先按“规范 ID”关联角色，再读取其展示名和档案；后续持久化会把规范 ID 与展示名分开保存。`,
