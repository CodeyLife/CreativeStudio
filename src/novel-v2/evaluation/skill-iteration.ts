...
import { buildSkillContextSections, createDatabaseSkillProvider, resolveStageSkillBundle } from "../skill-runtime";
...
export async function runSkillIteration(input: {
  ...
  // 1. 将实验 schema 作为 database SkillProvider，统一走 execution-point 解析。
  const skillProvider = createDatabaseSkillProvider(async () => {
    const skillResult = await workspace.query<SkillRow>(
      `SELECT skill_id, version, capabilities, applicable_tasks, required_memory_kinds,
              ...
              roles, depends_on, priority, content_fingerprint, source_ref
     FROM ${s}.skill_definitions
     WHERE enabled = TRUE
     ORDER BY skill_id`,
    );
    return skillResult.rows.map(mapSkillRow);
  });
  const skills = await skillProvider.list(workspace.projectId);
  ...
  // 3. 构造 prompt 并调用 LLM
  const iterationSkills = await resolveStageSkillBundle({
    projectId: workspace.projectId,
    provider: skillProvider,
    executionPoint: "skill.iteration",
    role: "skill-iterator",
    preflightId: `skill-iteration:${workspace.id}`,
  });
  ...
  const skillSections = buildSkillContextSections(iterationSkills, "skill.iteration", "Skill 迭代职责");
  const promptPackage = compileStageContext({
    ...
    skillManifest: iterationSkills.resolution,
    sections: [
      { id: "skill-iteration-evidence", kind: "review", title: "技能、审校证据与底层机制", text: prompt, priority: "required", provenanceRefs: [workspace.id, ...reviews.map((review) => review.id), learningAssessment?.id ?? ""] },
      ...skillSections,
    ],
  });
