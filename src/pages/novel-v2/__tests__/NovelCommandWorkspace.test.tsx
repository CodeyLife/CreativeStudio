...
function renderReviewedFinalProduction() {
  ...
  const workspace: NovelChapterWorkspace = {
    ...
    review: {
      ...
      complete: true,
      overallScore: 4.1,
      reviewerRoles: ["prose-reviewer"],
      reviewedAt: "2026-07-29T09:00:00.000Z",
      stale: false,
      issues: [
        { id: "issue-1", fingerprint: "fp-1", severity: "major", title: "人物反应太直白", evidenceQuote: "第二段。", paragraph: 2, revisionRanges: [{ start: 2, end: 2 }], suggestion: "改为动作呈现", sourceRoles: ["prose-reviewer"], status: "pending", updatedAt: "2026-07-29T09:00:00.000Z" },
...
describe("Novel command workspace", () => {
  ...
  it("summarizes the latest running workflow progress without a generic busy state", () => {
    const detail = deriveCurrentProgressDetail(
      ...
      [{ id: "rv-structure", artifactId: "draft", reviewerId: "structure", identity: "internal", verdict: "passed", role: "structure-reviewer", issues: [], createdAt: 1 }],
  ...
  it("uses persisted reviewer scores and issues for the concrete quality report", () => {
    ...
    expect(quality.reviewers.find((item) => item.role === "prose-reviewer")?.score).toBe(4.5);
    expect(quality.reviewers.find((item) => item.role === "structure-reviewer")?.score).toBe(3.5);
