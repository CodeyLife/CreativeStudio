...
export function deriveCurrentProgressDetail(
  ...
  if (stage === "draft") {
    return { phaseIndex: 1, stage, stageLabel, eyebrow: "正文生成", title: "正在创作正文", next: "正文完成后会进入三类审校与必要修订。", facts, latestEventSummary };
  }
...
interface QualityData {
  overall: number | null;
  reviewers: { role: string; score: number | null }[];
  issues: QualityIssue[];
  reviewCount: number;
}
...
export function deriveQuality(run: NovelRunState | undefined, artifacts: NovelArtifactSummary[], reviews: NovelReviewSummary[] = []): QualityData {
  ...
  return { overall, reviewers: currentReviews.filter((review) => review.role).map((review) => ({ role: review.role!, score: typeof review.score === "number" ? review.score : null })), issues: uniqueIssues, reviewCount: currentReviews.length || reviewArtifacts.length };
}
    ...
    <section className="pb-card">
      ...
      {score == null && quality.reviewCount === 0 && quality.issues.length === 0 ? (
        ...
        <div className="pb-quality-dims">
          {quality.reviewers.map((reviewer) => (
            <div key={reviewer.role} className="pb-dim">
              <span className="pb-dim-label">{reviewer.role}</span>
              <span className="pb-dim-bar"><span className="pb-dim-fill" style={{ width: reviewer.score == null ? "0%" : `${(reviewer.score / 5) * 100}%` }} /></span>
              <span className="pb-dim-val">{reviewer.score == null ? "—" : reviewer.score.toFixed(1)}</span>
            </div>
          ))}
        </div>
      </div>}
...
function SnapshotReviewPanel({ projectId, documentId, workspace, state, manuscriptDirty, onLocate, onRepairStarted }: { projectId: string; documentId?: string; workspace?: NovelChapterWorkspace; state: ChapterWorkspaceState; manuscriptDirty: boolean; onLocate: (paragraph: number) => void; onRepairStarted: () => void }) {
  ...
  return <div className="pb-context-review">
    ...
    <div className="pb-context-dimensions">{review.reviewerRoles.map((role) => <Tag key={role}>{role}</Tag>)}</div>
