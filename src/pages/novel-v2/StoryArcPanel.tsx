...
type ThreadResponsibility = { threadRef: string; responsibility: string; nextAdvance: string };
type ArcPlan = { title: string; objective: string; entryState: string; centralConflict: string; development: string[]; resolution: string; exitState: string; threadResponsibilities: ThreadResponsibility[]; foreshadowingRefs: string[]; expectedChapterCount: number; phases: Array<{ title: string; objective: string }>; authorIntent?: string };
...
export default function StoryArcPanel({ projectId, onApplied }: { projectId: string; onApplied?: () => void }) {
  ...
  async function saveEdit() {
    ...
    await action("edit", async () => {
      ...
      const arc = { ...editArc.arc, threadResponsibilities: editArc.arc.threadResponsibilities };
      await readJson(`/v2/projects/${encodeURIComponent(projectId)}/story-arcs/${encodeURIComponent(editArc.id)}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ bundle: { arc, batch: { batchIndex: batch.batchIndex, startChapterIndex: batch.startChapterIndex, complete: batch.complete }, chapters } }) });
  ...
  function addChapter() {
    setEditArc((current) => current ? { ...current, chapters: [...current.chapters, { index: Math.max(0, ...current.chapters.map((chapter) => chapter.index)) + 1, title: "新章节", narrativeFunction: "transition", stateTransition: { before: "", after: "", evidence: "" }, scenes: [], continuityConstraints: [] }] } : current);
  }
  ...
  function updateThreadResponsibility(index: number, changes: Partial<ThreadResponsibility>) {
    setEditArc((current) => current ? { ...current, arc: { ...current.arc, threadResponsibilities: (current.arc.threadResponsibilities ?? []).map((item, at) => at === index ? { ...item, ...changes } : item) } } : current);
  }
  ...
  return <motion.section className="novel-arc-workspace" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
    ...
    <main className="novel-arc-reader">
      ...
      <section className="novel-arc-summary"><p>{selected.arc.objective}</p><dl><dt>入场状态</dt><dd>{selected.arc.entryState}</dd><dt>核心冲突</dt><dd>{selected.arc.centralConflict}</dd><dt>发展路径</dt><dd>{selected.arc.development.join(" → ")}</dd><dt>收束方式</dt><dd>{selected.arc.resolution}</dd><dt>离场状态</dt><dd>{selected.arc.exitState}</dd><dt>长线责任</dt><dd>{selected.arc.threadResponsibilities.map((item) => `${item.threadRef}：${item.responsibility}；下一推进：${item.nextAdvance}`).join("\n") || "无"}</dd></dl></section>
      <div className="novel-arc-chapters">{selected.chapters.map((chapter) => <article key={chapter.id ?? chapter.index}><header><span>{String(chapter.globalOrder ?? chapter.index).padStart(2, "0")}</span><div><h3>{chapter.title}</h3><p>{chapter.stateTransition.evidence}</p></div>{chapter.documentId && <Tag>已关联章节</Tag>}</header><div className="novel-arc-chapter-grid"><span><b>章节功能</b>{chapter.narrativeFunction || "未指定"}</span><span><b>状态边界</b>{chapter.stateTransition.before} → {chapter.stateTransition.after}</span><span><b>连续性</b>{chapter.continuityConstraints.join("；") || "无额外约束"}</span><span><b>未解决</b>{chapter.unresolvedAtClose?.join("；") || "无"}</span></div>{chapter.scenes.length > 0 && <details><summary>{chapter.scenes.length} 个场景</summary>{chapter.scenes.map((scene, index) => <p key={index}><b>{scene.title}</b> · {scene.situation} · {scene.outcome}</p>)}</details>}</article>)}</div>
    </main>
    ...
    <Modal title="编辑整弧蓝图" width={1040} open={Boolean(editArc)} onCancel={() => setEditArc(undefined)} onOk={() => void saveEdit()} confirmLoading={busy === "edit"} okText="保存并重新审核">
      {editArc && <div className="novel-arc-editor">
        ...
        <div className="novel-arc-editor-head"><strong>本弧长线责任</strong><Button icon={<PlusOutlined />} disabled={!canEditArcBoundary} onClick={() => updateArc({ threadResponsibilities: [...(editArc.arc.threadResponsibilities ?? []), { threadRef: "", responsibility: "", nextAdvance: "" }] })}>添加责任</Button></div>
        {editArc.arc.threadResponsibilities.map((item, index) => <section key={index}><header><strong>责任 {index + 1}</strong><Button danger icon={<DeleteOutlined />} disabled={!canEditArcBoundary} onClick={() => updateArc({ threadResponsibilities: editArc.arc.threadResponsibilities.filter((_, at) => at !== index) })} /></header><Input disabled={!canEditArcBoundary} value={item.threadRef} placeholder="剧情线规范 ID" onChange={(event) => updateThreadResponsibility(index, { threadRef: event.target.value })} /><Input.TextArea disabled={!canEditArcBoundary} value={item.responsibility} placeholder="本弧负责保持、推进、交汇、退出或转化什么" onChange={(event) => updateThreadResponsibility(index, { responsibility: event.target.value })} /><Input.TextArea disabled={!canEditArcBoundary} value={item.nextAdvance} placeholder="下一次可验证推进或触发条件" onChange={(event) => updateThreadResponsibility(index, { nextAdvance: event.target.value })} /></section>)}
        ...
        <div className="novel-arc-editor-head"><strong>阶段</strong><Button icon={<PlusOutlined />} disabled={!canEditArcBoundary} onClick={() => updateArc({ phases: [...editArc.arc.phases, { title: "新阶段", objective: "" }] })}>添加阶段</Button></div>
        {editArc.arc.phases.map((phase, index) => <section key={index}><header><strong>阶段 {index + 1}</strong><Button danger icon={<DeleteOutlined />} disabled={!canEditArcBoundary} onClick={() => updateArc({ phases: editArc.arc.phases.filter((_, at) => at !== index) })} /></header><Input disabled={!canEditArcBoundary} value={phase.title} placeholder="阶段标题" onChange={(event) => updatePhase(index, { title: event.target.value })} /><Input value={phase.objective} disabled={!canEditArcBoundary} placeholder="阶段目标" onChange={(event) => updatePhase(index, { objective: event.target.value })} /></section>)}
        <div className="novel-arc-editor-head"><strong>当前批次章节蓝图</strong><Button icon={<PlusOutlined />} disabled={editArc.chapters.length >= 16} onClick={addChapter}>添加章节</Button></div>{editArc.chapters.map((chapter, index) => <section key={chapter.id ?? index}><header><strong>第 {index + 1} 章</strong><Space><Button icon={<ArrowUpOutlined />} disabled={index === 0} onClick={() => moveChapter(index, -1)} /><Button icon={<ArrowDownOutlined />} disabled={index === editArc.chapters.length - 1} onClick={() => moveChapter(index, 1)} /><Button danger icon={<DeleteOutlined />} disabled={editArc.chapters.length <= 1 || Boolean(chapter.documentId)} onClick={() => setEditArc({ ...editArc, chapters: editArc.chapters.filter((_, at) => at !== index) })} /></Space></header><Input value={chapter.title} placeholder="章节标题" onChange={(event) => updateChapter(index, { title: event.target.value })} /><Input value={chapter.narrativeFunction ?? ""} placeholder="章节功能（可选）" onChange={(event) => updateChapter(index, { narrativeFunction: event.target.value || undefined })} /><Input value={chapter.povCharacterId ?? ""} placeholder="POV 角色（可选）" onChange={(event) => updateChapter(index, { povCharacterId: event.target.value.trim() || undefined })} /><Input.TextArea value={chapter.stateTransition.before} placeholder="起始状态边界" onChange={(event) => updateChapter(index, { stateTransition: { ...chapter.stateTransition, before: event.target.value } })} /><Input.TextArea value={chapter.stateTransition.after} placeholder="结束状态边界" onChange={(event) => updateChapter(index, { stateTransition: { ...chapter.stateTransition, after: event.target.value } })} /><Input.TextArea value={chapter.stateTransition.evidence} placeholder="状态变化证据或保持稳定的观察边界" onChange={(event) => updateChapter(index, { stateTransition: { ...chapter.stateTransition, evidence: event.target.value } })} /><Input.TextArea value={chapter.continuityConstraints.join("\n")} placeholder="连续性约束，每行一项" onChange={(event) => updateChapter(index, { continuityConstraints: csv(event.target.value) })} /><Input.TextArea value={(chapter.unresolvedAtClose ?? []).join("\n")} placeholder="结束时未解决事项，每行一项" onChange={(event) => updateChapter(index, { unresolvedAtClose: csv(event.target.value) })} /><div className="novel-arc-scenes"><strong>场景</strong>{chapter.scenes.map((scene, sceneIndex) => <div key={sceneIndex}><Input value={scene.title} placeholder="场景标题" onChange={(event) => updateChapter(index, { scenes: chapter.scenes.map((item, at) => at === sceneIndex ? { ...item, title: event.target.value } : item) })} /><Input.TextArea value={scene.situation} placeholder="场景处境" onChange={(event) => updateChapter(index, { scenes: chapter.scenes.map((item, at) => at === sceneIndex ? { ...item, situation: event.target.value } : item) })} /><Input.TextArea value={scene.observableActions.join("\n")} placeholder="可观察行动，每行一项" onChange={(event) => updateChapter(index, { scenes: chapter.scenes.map((item, at) => at === sceneIndex ? { ...item, observableActions: csv(event.target.value) } : item) })} /><Input.TextArea value={scene.outcome} placeholder="场景结果" onChange={(event) => updateChapter(index, { scenes: chapter.scenes.map((item, at) => at === sceneIndex ? { ...item, outcome: event.target.value } : item) })} /><Button danger icon={<DeleteOutlined />} onClick={() => updateChapter(index, { scenes: chapter.scenes.filter((_, at) => at !== sceneIndex) })} /></div>)}<Button icon={<PlusOutlined />} onClick={() => updateChapter(index, { scenes: [...chapter.scenes, { title: "新场景", participants: [], situation: "", observableActions: [], outcome: "" }] })}>添加场景</Button></div></section>)}</div>}
    </Modal>
