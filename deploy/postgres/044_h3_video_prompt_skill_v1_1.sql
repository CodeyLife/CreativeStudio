-- h3-video-prompt v1.1.0: chapter-level unified subject library (chapter-library).
-- Segments stop re-declaring subjectDefinitions; they reference the chapter-wide
-- <Subject N> library via subjectLabels, and prompt assembly injects only the
-- referenced definitions so each segment prompt stays self-contained.
-- 043 stays immutable history; this row upserts the runtime descriptor to match
-- skills/novel-v2/h3-video-prompt.yaml.
