-- h3-video-prompt v1.4.1：切点格式精确化 + 标签定义显式化（两执行点同步）
-- 根因：真实生成验证（2026-08-31）发现 provider 采用前缀式切点写法（At MM:SS.mmm cut to [Shot N]），
-- 旧规则「以 At MM:SS.mmm 格式的切点开始」语义含糊，无法推断时间戳须紧跟镜头标记之后；
-- repair 消息「开场镜头不得携带时间戳」对前缀式写法的模型反直觉，三轮修复均无法纠正。
-- v1.4.1 给出精确格式示例（[Shot 2] At 00:02.000, ...）并显式排除前缀式写法；
-- short.script 执行点同时显式化标签规则：正文用到的每个 <Subject N> 须在本段定义，
-- 顶层 characters 不会自动成为标签。
-- 配套：chapter-script-h3.ts collectShotTimeline 改为最近邻配对，兼容前缀式写法（双惯例支持）。
-- 043-051 保持不可变历史；本迁移 upsert 运行时 descriptor，与 YAML 对齐。

INSERT INTO skill_definitions(
  skill_id, version, capabilities, applicable_tasks, execution_points,
  roles, depends_on, quality_gates, prompt_sections, priority,
  content_fingerprint, source_ref, enabled
)
VALUES (
  'h3-video-prompt',
  '1.4.1',
