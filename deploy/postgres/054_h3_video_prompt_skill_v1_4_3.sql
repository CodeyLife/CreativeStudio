-- h3-video-prompt v1.4.3：片段时长分布契约（两执行点同步）
-- 根因：v1.4.2 放宽片段上限到 15s 后，实测（2026-08-31，契约 v4 产物）模型把
-- 5-15s 区间理解为均匀中值——30s 目标交付 [8,7,9]，冲击场面画面无法充分展开。
-- v1.4.3 明确时长按信息密度取值：宏大场面/战斗交锋/冲击性瞬间取区间上沿
--（约 12-15s），只有快节奏对话交锋与短促反应镜头才用短片段（5-8s），
-- 禁止整体贴下限。
-- 配套：chapter-script-h3 SCRIPT_CONTRACT_VERSION v7→v8、
-- SHORT_SCRIPT_CONTRACT_VERSION v4→v5（分布契约变化使旧产物指纹失效重生成），
-- 两侧任务 prompt 同步注入分布指引。
-- 043-053 保持不可变历史；本迁移 upsert 运行时 descriptor，与 YAML 对齐。
