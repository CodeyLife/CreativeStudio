-- h3-video-prompt v1.1.0: 影视镜头语言契约（方法论沉淀自 .agents/skills/short-drama-writing）——
-- 镜头四要素（景别/角度/运镜/光线氛围）、片段节奏范式（峰+刹车）、signature shot 引导（不强制）、
-- 题材视觉特效具体化、完整风格句。043-047 保持不可变历史；本迁移 upsert 运行时 descriptor。
