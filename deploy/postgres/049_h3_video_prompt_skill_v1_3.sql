-- h3-video-prompt v1.3.0：剧集剧作层契约（方法论沉淀自 .agents/skills/short-drama-writing/references/dramaturgy.md）
-- ——开场即冲突、情绪节点节奏、出口即钩子（末段冲击瞬间切卡）、台词密度三功能、
-- 反转须有伏笔（plant → overlook → detonate）、人物经济（核心三角）；
-- 同时新增 short.script 执行点（novel_short_script_h3 创意短剧入口，无定稿正文、
-- 从核心创意自拟节拍）。043-048 保持不可变历史；本迁移 upsert 运行时 descriptor，
-- 与 skills/novel-v2/h3-video-prompt.yaml 对齐。
...
VALUES (
  ...
  'required',
  '',
  'database:049_h3_video_prompt_skill_v1_3.sql',
  TRUE
)
