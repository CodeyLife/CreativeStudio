-- 回滚迁移：h3-video-prompt 恢复 v1.0.0（撤销章节级统一定义库抽象，恢复片段自包含六段契约）。
-- 043（v1.0 播种）/ 044（v1.1 library）/ 045（v1.2 五段交付）保持不可变历史；
-- 本迁移 upsert 把运行时 descriptor 恢复为 v1.0.0，与 skills/novel-v2/h3-video-prompt.yaml 对齐。
...
VALUES (
  ...
  'required',
  '',
  'database:046_h3_video_prompt_skill_rollback_v1.sql',
  TRUE
)
