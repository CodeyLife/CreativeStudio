...
本文是 `docs/novel-v2/中文小说创作参考.md` 在当前代码中的落地索引。它描述当前三角色版本的实际调用关系、职责边界和修改后的验证方式，不是另一套运行时配置。
...
| --- | --- | --- | --- | --- |
| `prose-craft@1.3.1` | 场景具象化、读者现场复原、POV/叙述距离、段落同构、抽象表达过密、声部趋同和中文句法 | 把蓝图转成可经历的动作、感官、空间、对白和判断；技术认知只有在改变即时选择、表达独有认知或承担不可替代世界观功能时才承担正文信息，不能成为当前动作的唯一主语/原因/结果 | 只有出现正文损害时，引用证据识别摘要替代、同构、越界、声部问题或读者无法复原现场的问题；使用 `readerReconstruction` 保存缺失证据和 blocked question | 先修场景/视角/声部根因，再处理句法与词语；不能只换术语，必须补足必要的可观察证据并删除不改变事实、选择和因果的重复标签；不使用术语词表或抽象密度阈值 |
