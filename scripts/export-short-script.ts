/**
 * 把 MCP 短剧生成的报告 JSON（tmp/mcp-smoke-report.json）导出为一份
 * 独立可读的 HTML 文件（emerald + 深色风格），便于脱离 JSON 直接浏览/分享。
 *
 * 用法：npx tsx scripts/export-short-script.ts [报告路径] [输出目录]
 * 默认读取 tmp/mcp-smoke-report.json，输出到 outputs/novel-v2-short-scripts/<scriptId>.html
 */
...
function renderBeats(beats: Beat[]): string {
  return beats.map((b) =>
    `<li><span class="beat-kind">${escapeHtml(b.kind)}</span> <span class="beat-id">${escapeHtml(b.id)}</span> — ${escapeHtml(b.summary)}</li>`,
  ).join("");
}

function renderChars(chars: Char[]): string {
  if (!chars.length) return `<p class="muted">（无角色基线）</p>`;
  return chars.map((c) =>
    `<li><strong>${escapeHtml(c.name)}</strong><br><span class="muted">${escapeHtml(c.appearanceEn)}</span></li>`,
  ).join("");
}

function renderHints(hints: string[]): string {
  if (!hints.length) return `<p class="muted">（无）</p>`;
  return hints.map((h) => `<li>${escapeHtml(h)}</li>`).join("");
}
...
<div class="wrap">
  ...
  <div class="card">
    <h2>创意 & 导演指令</h2>
    <p><b>核心创意：</b>${escapeHtml(report.idea ?? "")}</p>
    ${report.instruction ? `<p><b>导演指令：</b>${escapeHtml(report.instruction)}</p>` : ""}
  </div>
